...
import refresh_token_val from "../refreshTokenObject";
...
export default class OpenId extends AuthType {
        ...
        this.server.auth.scheme('security_access_control_scheme', (server, options) => ({
            authenticate: async (request, h) => {
                ...
                try {
                    ...
                    //server.log(['error', 'security'], `Error while validating the session cookie: ${error}`);

                    /*Commented to stop Kibana getting struck as CORS policy comes into effect as it stops any outgoing kibana request to keycloak.
                      On invalid token or failed to update access_token using refresh_token the code will reload Kibana with new token till IDP SSO
                      is active. Once IDP SSO timeout the code will redirect to the login page for a fresh login.
                    */

                    /*if (this._cookieValidationError) {
                      server.log(['error', 'security'], `Cookie Validation Error`);
                    ...
                    if (request.headers) {
                        //server.log(['error', 'security'], `Headers exist... send a 401?`);
    ...
    onUnAuthenticated(request, h, error) {
        ...
        if (error instanceof MissingTenantError) {
            ...
            this.server.log(['error', 'security'], `Error occured with authentication: ${error}`);
            return h.redirect(this.basePath + '/customerror?type=authError')
        }

        //this.server.log(['error', 'security'], `onAuthenticated - Redirect`);
    ...
    sessionValidator(server) {
        ...
        let validate = async (request, session) => {
            ...
            if (session.exp && session.exp <= Math.floor(Date.now() / 1000)) {
                ...
                let refresh_token = refresh_token_val.getRefresh_token();
                if (refresh_token && refresh_token !== null && refresh_token !== '') {
                    try {
                        let body = await this.refreshTokenRequest(refresh_token);
