...
module.exports = async function (pluginRoot, server, kbnServer, APP_ROOT, API_ROOT, openIdEndPoints) {
    ...
    server.ext('onPreResponse', function (request, h) {
        ...
        if (request.response.isBoom && request.path.indexOf(`${APP_ROOT}${routesPath}login`) > -1 && request.response.output.statusCode === 500) {
            server.log(['error', 'security'], `Error occured while obtaining token: ${request.response}`);
    ...
    try {
        ...
        server.route({
            ...
            handler: async (request, h) => {
                ...
                // Saving the refresh_token from the request
                try {
                    refresh_token_val.setRefresh_token(request.auth.credentials.refreshToken);
                } catch (error) {
                    server.log(['error', 'security routes'], `Error during in-memory storing of refresh_token: ${error}`);
                }

                let nextUrl = (credentials.query && credentials.query.nextUrl) ? credentials.query.nextUrl : null;

                try {
                    // Bell gives us the access token to identify with here, but we want the id_token returned from the IDP
                    ...
                    server.log(['error', 'security routes'], `Error: ${error}`);
